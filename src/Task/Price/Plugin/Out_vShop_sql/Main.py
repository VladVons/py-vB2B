...
class TSql(TSqlBase):
    ...
    def Product_Create(self, aDbl: TDbProductEx) -> str:
        ...
        def Product(aData: list, _aMax: int) -> str:
            ...
            for Row in aData:
                DbRec.Data = Row
                Value = f'({DbRec.id}, {self.Conf.TenantId}, {bool(DbRec.available)})'
                Values.append(Value)
        ...
        def Product_Lang(aData: list, _aMax: int) -> str:
            ...
            for Row in aData:
                DbRec.Data = Row
                Name = DbRec.name.translate(self.Escape)
                ...
                Value = f"({DbRec.id}, {self.Conf.LangId}, '{Name}', '{Descr}', '{Feature}')"
                Values.append(Value)
        ...
        def Product_ToCategory(aData: list, _aMax: int) -> str:
            ...
            for Row in aData:
                DbRec.Data = Row
                Values.append(f'({DbRec.id}, {DbRec.category_id})')
        ...
        def Product_Price(aData: list, _aMax: int) -> str:
            ...
            for Row in aData:
                DbRec.Data = Row
                Value = f'({DbRec.id}, {self.Conf.PriceId}, {DbRec.price})'
                Values.append(Value)
